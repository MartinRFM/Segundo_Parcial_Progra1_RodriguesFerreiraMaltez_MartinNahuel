...
class FormMainMenu(Form):
    ...
    def __init__(self, name, pantalla, x, y, active, level_num, music_path):
        ...
        self.menu_ppal_title = TextTitle(x=DIMENSION_PANTALLA[0]//2, y=DIMENSION_PANTALLA[1]//2-250, texto='THIS OR THAT', pantalla=pantalla, font_size=90)
        self.menu_ppal_subtitle = TextTitle(x=DIMENSION_PANTALLA[0]//2, y=DIMENSION_PANTALLA[1]//2-160, texto='MENU PRINCIPAL', pantalla=pantalla, font_size=60)
        ...
        self.widget_list = [
            self.menu_ppal_subtitle, self.menu_ppal_title, self.button_exit,
            self.button_options, self.button_rankings, self.button_start
        ]
        
        self.music_update()
    
    def click_start(self, _):
        self.surface = pg.image.load('assets/img/certamen.png').convert_alpha() 
        self.widget_list = []
